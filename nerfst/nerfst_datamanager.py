...
@dataclass
class NerfSTDataManagerConfig(VanillaDataManagerConfig):
    """Configuration for the InstructNeRF2NeRFDataManager."""

    _target: Type = field(default_factory=lambda: NerfSTDataManager)
    ref: str = "styles/14.jpg"
    """reference style image"""

class NerfSTDataManager(VanillaDataManager):
    """Data manager for InstructNeRF2NeRF."""

    config: NerfSTDataManagerConfig
    ...
    def setup_train(self):
        ...
        # save style image
        self.train_dataset.scale_factor
        
        self.style_image = prepare_style_image(self.config.ref, self.train_dataset.scale_factor, self.train_dataparser_outputs.alpha_color) # h, w, c

    def next_train(self, step: int) -> Tuple[RayBundle, Dict]:
        """Returns the next batch of data from the train dataloader."""
        self.train_count += 1
        assert self.train_pixel_sampler is not None
        # self.image_batch是所有图片的缓存
        # 每次从所有图片中随机采样像素
